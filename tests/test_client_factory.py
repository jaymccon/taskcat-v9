...
class TestBoto3Cache(unittest.TestCase):
    ...
    def test__get_account_info(self, mock_session, mock__get_region):
        ...
        sts.get_caller_identity.side_effect = ClientError(error_response={"Error": {"Code": "test"}}, operation_name="test")
        with self.assertRaises(ClientError):
            cache._get_account_info("default")

        sts.get_caller_identity.side_effect = ClientError(error_response={"Error": {"Code": "AccessDenied"}}, operation_name="test")
        ...
        sts.get_caller_identity.side_effect = ProfileNotFound(profile="non-existent_profile")
        with self.assertRaises(TaskCatException):
            cache._get_account_info("default")